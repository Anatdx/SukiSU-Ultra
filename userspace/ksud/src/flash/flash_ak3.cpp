...
std::string get_ak3_info(const std::string& zip_path) {
    ...
    auto result = exec_command({
        "unzip", "-o", "-j", zip_path, "anykernel.sh", "-d", temp_dir
    });
 ...
 */
Ak3FlashResult flash_ak3(const Ak3FlashConfig& config,
                          Ak3LogCallback log_callback,
    ...
    std::string binary_path = std::string(AK3_WORK_DIR) + "/META-INF/com/google/android/update-binary";
    ...
    if (pid == 0) {
        ...
        execl("/system/bin/sh", "sh", binary_path.c_str(),
              "3", std::to_string(pipefd[1]).c_str(), work_zip.c_str(),
              nullptr);
