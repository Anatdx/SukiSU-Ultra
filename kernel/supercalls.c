...
static void init_uid_scanner(void)
{
	...
	if (ksu_uid_scanner_enabled) {
		...
		if (ret != 0) {
			pr_err(
			    "Failed to initialize throne communication: %d\n",
			    ret);
		}
	}
}
...
static int do_enable_uid_scanner(void __user *arg)
{
	...
	case UID_SCANNER_OP_TOGGLE: {
		...
		if (enabled == ksu_uid_scanner_enabled) {
			pr_info("enable_uid_scanner: no need to change, "
				"already %s\n",
		...
		if (enabled) {
			...
			int ret = ksu_throne_comm_init();
			if (ret != 0) {
				pr_err("enable_uid_scanner: failed to "
				       "initialize: %d\n",
				       ret);
				return -EFAULT;
			}
...
void ksu_supercalls_init(void)
{
	...
	// SuperKey prctl kprobe - always register regardless of SUSFS
#ifdef CONFIG_KSU_SUPERKEY
...
void ksu_supercalls_exit(void)
{
	...
	// SuperKey prctl kprobe - always unregister regardless of SUSFS
#ifdef CONFIG_KSU_SUPERKEY
