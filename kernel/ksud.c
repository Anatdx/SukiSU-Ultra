			     ...
			     struct user_arg_ptr *envp, int *flags)
{
	...
	// Debug: log first few execve paths during boot
	{
		static int execve_log_count = 0;
		if (execve_log_count < 10 && filename->name) {
			pr_info("kprobe execve path: %s (pid=%d)\n",
