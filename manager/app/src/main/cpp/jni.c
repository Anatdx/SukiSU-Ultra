...
NativeBridge(getAppProfile, jobject, jstring pkg, jint uid) {
  ...
  jclass cls =
      GetEnvironment()->FindClass(env, "com/sukisu/ultra/Natives$Profile");
  jmethodID constructor =
...
NativeBridgeNP(isKPMEnabled, jboolean) { return is_KPM_enable(); }
...
NativeBridge(verifyModuleSignature, jboolean, jstring modulePath) {
  ...
  const char *cModulePath =
      GetEnvironment()->GetStringUTFChars(env, modulePath, nullptr);
